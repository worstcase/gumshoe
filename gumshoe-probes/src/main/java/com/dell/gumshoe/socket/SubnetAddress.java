...
public class SubnetAddress implements SocketMatcher {
    public static final SocketMatcher ANY = new MatchAny();
    ...
    public String toString() {
        ...
        for(int i=0;i<4;i++) {
            if(i>0) out.append(".");
            out.append(0xFF & (int)targetAddress[i]);
        }
        out.append("/");
        for(int i=0;i<4;i++) {
            if(i>0) out.append(".");
            out.append(0xFF & (int)mask[i]);
        }
        out.append(":");
    ...
    private static class MatchAny implements SocketMatcher {
