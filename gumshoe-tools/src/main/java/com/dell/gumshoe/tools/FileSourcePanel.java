...
public class FileSourcePanel extends JPanel {
    ...
    private class FileOpener extends SwingWorker<Map<Stack,StatisticAccumulator>,Object> {
        private final boolean forward;
        public FileOpener(boolean forward) {
            this.forward = forward;
        }
        @Override
        public Map<Stack,StatisticAccumulator> doInBackground() throws Exception {
            return forward ? file.getNextSample() : file.getPreviousSample();
        }
        @Override
        public void done() {
            try {
                final Map<Stack,StatisticAccumulator> sample = get();
                if(sample==null) {
